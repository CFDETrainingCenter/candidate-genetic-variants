---
title: "Protein Knowledge and Research Context: IDG/Pharos"
aliases:
  - /pharos.html
---
...
Use Pharos to connect the paper's genes with protein knowledge and identify
...
about human proteins and drug targets. It brings protein identifiers,
annotations, publications, interactions, ligands, drugs, and target-development
categories into one resource.

### Why use Pharos for this paper?

The paper reports candidate variants in 25 protein-coding genes. GTEx and
HuBMAP showed where those genes have indexed expression. Pharos adds a different
question: what protein knowledge and experimental starting points have already
been recorded for each gene?
...
TDL is a protein-level field. The study classification remains the
variant-level field.

::: {.callout-tip title="Why Pharos helps"}
The expression resources provide tissue and cell-type context. Pharos adds
protein identifiers, knowledge categories, and counts of linked publications,
interactions, ligands, and drugs. These fields help identify which target
...
### Request protein-target context
...
Each row represents one protein target. The preview connects its gene symbol
and UniProt identifier to a TDL. The other fields count publications,
protein-interaction records, ligands, and drugs linked to the target in Pharos.

TDL and the count fields answer related but different questions. TDL follows
Pharos development criteria. The counts describe records attached to the
target. A Tchem target can therefore have ligand records while its drug count
is zero.
...
Count the TDL categories and compare protein fields that can inform research
follow-up.
...
The `genes` column counts the queried targets assigned to each Pharos TDL. In
the dated teaching data, 18 targets are Tbio, 4 are Tclin, and 3 are Tchem. No
Tdark targets were returned. The Tclin targets are *TTR*, *DMD*, *KCNQ1*, and
*MYH7*; the Tchem targets are *MYLK3*, *POLG*, and *TNNI3K*.
...
Each row combines a target's TDL with its linked record counts. The contrasting
patterns suggest different starting points:
...
The dated response returns the four Tclin targets, *TTR*, *DMD*, *KCNQ1*, and
*MYH7*, plus the Tchem targets *MYLK3* and *TNNI3K*. *POLG* remains visible in
the complete table with two ligand records and no drug records.
...
**Next:** Join the GTEx, HuBMAP, and Pharos evidence back to all 54 variant rows
and use the combined context to prioritize research follow-up.
