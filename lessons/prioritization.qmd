...
classifications reported in the paper remain attached to each variant.
...
Each column retains its original meaning and supports the question it can
answer. Together, the columns form a qualitative framework for experimental
follow-up.
...
selected cell type. The other 13 rows remain in the complete table for clinical
...
*TNNT2* variants. These findings support the relevance of *TNNT2* to DCM in
this cohort. Variant-specific evidence is still needed for p.Asp259Ala.

The source study's protein-effect score used PolyPhen, SIFT, REVEL, and CADD.
ProtVar adds two newer models for comparison: AlphaMissense predicts that the
substitution may affect protein function, and EVE returns an uncertain
prediction. FoldX estimates a small overall protein-stability change for the
dated teaching result.

The AlphaFold pLDDT score reports confidence in the local structural model
around residue 259. AlphaMissense, EVE, and FoldX address properties of the
...
Pharos identifies cardiac troponin T as a biologically characterized protein
with extensive interaction records. Cardiac troponin T interacts with troponin
I, troponin C, and tropomyosin to regulate thin-filament contraction.
...
supports cardiac follow-up. Variant-specific experiments address the mechanism.
...
| VUS with cardiac expression support | *TNNI3* `c.337G>A`; *ACTC1* `c.1132T>C`; *TNNT2* `c.776A>C`; *FLNC* `c.4181A>G`; *TNNI3K* `c.827+1G>T` | These remain VUS. Cardiac expression supports studying the genes, and variant-specific experiments can test the effect of each allele. |
...
above zero in 0.4% of ventricular cardiac myocytes. The P/LP findings above have
stronger support from this selected cell type.
...
6. Important evidence still needed.
...
- Evidence still needed is stated explicitly.
...
- Tissue expression, cell-type measurements, and protein information guide
  selection of candidate genetic variants and models while preserving study
  classifications.
