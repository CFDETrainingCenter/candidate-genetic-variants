...
The paper provides 54 classified variant rows. GTEx, HuBMAP, and Pharos describe
the 25 associated genes. These sources can share a table, but gene-level results
do not change a variant classification.
...
|---|---|---|
| Paper fields | The exact variant, phenotype, score, and class reported by the study | Which published candidate genetic variant and phenotype should guide the interpretation? |
...
the best candidate genetic variant. The goal is a justified next step with visible source data.
...
The 25-row `gene_context` table contains GTEx median TPM, HuBMAP expression and
...
The matrix places each variant beside the corresponding GTEx, HuBMAP, and Pharos
...
The full matrix could support many questions. This analysis focuses on dilated
...
The filter retains 15 of the 28 DCM rows. Ten are P or LP in the paper, and five
are VUS. The other 13 have not become less important or less plausible. They are
absent only because the selected HuBMAP index did not return a ventricular
cardiac-myocyte value.

## Examine one missense variant with ProtVar

The paper classified *TNNT2* `c.776A>C` (p.Asp259Ala) as a VUS. Two other
participants with DCM carried pathogenic or likely pathogenic *TNNT2* variants.
These findings support the relevance of *TNNT2* to DCM in this cohort, but they
do not determine the effect of p.Asp259Ala.
...
ProtVar provides variant-level evidence for p.Asp259Ala. AlphaMissense predicts
that the substitution may affect protein function, while FoldX does not predict
a large change in overall protein stability. The next step would be to compare
wild-type and D259A troponin complexes across calcium concentrations to test
whether the substitution changes calcium-regulated thin-filament activity. The
study classification remains VUS.
...
The study classification remains the variant-level anchor. The API results help
select genes and experimental systems for follow-up. This distinction follows
the precision approach used by
[Birch and colleagues](https://doi.org/10.1186/s12967-025-07586-w): integrate
variant classification, phenotype concordance, and molecular evidence while
keeping confidence in each finding explicit.
...
| Cardiac expression present, variant unresolved | *TNNI3* `c.337G>A`; *ACTC1* `c.1132T>C`; *TNNT2* `c.776A>C`; *FLNC* `c.4181A>G`; *TNNI3K* `c.827+1G>T` | These remain VUS. Cardiac expression supports studying the genes but does not establish the effect of an allele. |
...
Within this analysis, findings in *DES*, *TNNT2*, *MYBPC3*, *ACTC1*, *PLN*, and
*MYLK3* have the clearest rationale for variant-specific cardiac cellular
models. The results also identify possible perturbation tools and follow-up
questions.
:::
...
relationship. These records suggest possible perturbation tools for testing the
proposed mechanism.

Pharos classifies *TNNI3K* as `Tchem`, but the `c.827+1G>T` VUS gene was detected
in only 0.4% of sampled ventricular cardiac myocytes. Its chemical records may
still be useful, but these HuBMAP results provide less support for prioritizing
...
The 13 DCM rows absent from `dcm_follow_up` remain in the full matrix. They
include P/LP findings in *MYL3*, *TTN*, *GYG1*, *LMNA*, and *DMD*, plus one
*LMNA* VUS. HuBMAP did not return values for the selected cell type, so these
genes require another atlas or experimental system.
...
`c.323A>G` each appear in two. Recurrence tells us about frequency within this
study. Its importance still depends on variant-specific evidence.
