...
Sarcomeric variants were most common in the first two groups, while desmosomal
variants were concentrated in the third. The teaching table includes *MYH7*,
*MYBPC3*, *TNNT2*, *ACTC1*, *DSG2*, *DSC2*, and *PKP2*.

HuBMAP adds reference cell-type expression data to the follow-up analysis. We
begin with ventricular cardiac myocytes because the ventricles provide the
heart's main pumping force.

Many candidate genes contribute to cardiac-muscle contraction or structure.
This cell-type choice also follows the GTEx left-ventricle comparison.
...
Treat this arbitrarily ordered subset as a feasibility screen; low, zero, or
unavailable values do not establish biological absence.
...
The Cells API returns values for a subset of genes. Record genes outside that
subset as unavailable and keep them distinct from a measured zero.
...
up to the first 500 records, and requests one gene at a time. An unavailable
result records the API response separately from a measured zero.
...
percentage of those records with a value above zero.
...
Keep the ventricular cardiac-myocyte rows and count genes with returned and
unavailable expression values. Start with coverage before comparing
measurements.
...
*TNNT2* ranks second by percent detected and sixth by mean expression.
*DES* ranks second by mean expression and third by percent detected.
...
The plot shows the 10 highest means from the complete 14-gene table. The table
above includes all 14 genes with returned measurements.
...
- Every retrieved record has the same expression value
  ...
  > A mean and a percentage above zero summarize variation across the retrieved
  > records.

- The gene is unavailable in the Cells API index

  > An unavailable gene has an empty set of returned values, so this comparison
  > contains genes with measurements.
...
- The expression index returned an unavailable result

  > Correct. Keep this result separate from a returned value of zero.

- Every indexed record had a measured value of zero

  > Choose unavailable because that label records an empty API result for this
  > gene.
...
- The wrapper calculates mean normalized expression and the percentage of
  returned records above zero. These summaries can produce different gene
  rankings.
