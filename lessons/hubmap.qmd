...
Move from bulk heart tissue to ventricular cardiac myocytes. Compare the mean
returned value with the percentage of records above zero.
...
The paper did not analyze cell types, so HuBMAP adds new evidence rather than
...
average returned values with how often those values are above zero.
:::
...
One row represents one queried gene. `mean_normalized_expression` is the average
returned value, and `percent_detected` is the percentage of retrieved records
above zero. The separate `availability` field states whether the index returned
any values.
...
We can now ask which genes have data, how large their returned values are, and
...
In the dated teaching data, *MYH7* and *DES* have the highest mean returned
values. Their values are above zero in 39.8% and 35.2% of the retrieved records.
The mean describes magnitude, while the percentage above zero describes
frequency.
