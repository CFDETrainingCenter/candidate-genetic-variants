...
: A framework that classifies sequence variants as pathogenic, likely
  ...
  begins with variants that have already been evaluated and retains those
...
: A table operation in which several variant rows can match one gene row. Used
  correctly, it preserves every original variant while attaching shared
...
: Clinical classification categories indicating different strengths of
  evidence that a variant contributes to disease. These labels remain attached
  to the prior evaluation throughout this module. [Learn more about the
...
: A TDL for targets with substantial biological or disease-related knowledge
  within the Tbio criteria. These targets may need
